...
if [ ! -d "${SUIF_INSTALL_INSTALL_DIR}/IntegrationServer" ]; then
    ...
    #readSecretFromUser "Insert IS setup time Administrator password"
    #SUIF_INSTALL_TIME_ADMIN_PASSWORD="${secret}"
    #unset secret

    # Parameters - applySetupTemplate
    # $1 - Setup template directory, relative to <repo_home>/02.templates/01.setup
    applySetupTemplate "MSR/1015/cu_cds_jdbc_mq" || tail -f /dev/null # exit 6

    #unset SUIF_INSTALL_TIME_ADMIN_PASSWORD
fi
